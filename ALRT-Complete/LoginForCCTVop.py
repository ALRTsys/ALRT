...
mydb = mysql.connector.connect(
    host= 'localhost',
    user= 'usually root but if changed put yours',
...
# Login Page
class CCTVopLoginApp(ft.UserControl):
    def __init__(self):
        super().__init__()
        self.FAILED_TIMES = 1
    ...
    def go_contact(self, e):
        self.page.route = "/contactCCTVopLogin"
        self.page.update()
    ...
    def go_LoginForAdmin(self, e):
        self.page.route = "/LoginForAdmin"
        self.page.update()
    ...
    def verify(self, e):
        ...
        sql = "SELECT * FROM cctv_operator WHERE BINARY Username ='%s' AND BINARY Password =sha1('%s')" % (
        ...
        if mycursor.fetchone():
                if email_value != 'Admin':
                    # go to CCTV oprtor monitor page
                    print("---- login success ---")
                    self.page.session.set("login", True)
                    self.page.route = "/MonitorPage"
                    self.log.write(time.strftime('%H:%M:%S %p  %x') + ' CCTV operator login' + '\n')
                    self.page.update()
            ...
            self.header = ft.Container(
                    content=ft.Row(
                            ...
                            controls = [
                                ...
                                ft.TextButton(text="Sign in As Admin?", on_click=self.go_LoginForAdmin, style=ft.ButtonStyle(color=ft.colors.WHITE)),
                            ]),
                    padding=0,
